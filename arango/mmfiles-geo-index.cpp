...
int GeoIndexNewPot(GeoIx* gix) {
  ...
  if (gix->pots[0].LorLeaf == 0) {
    ...
    int newpotct = (int)x;
    gp = static_cast<GeoPot*>(realloc(gix->pots, newpotct * sizeof(GeoPot)));
...
GeoIdx* GeoIndex_new(void) {
  ...
  gix->pots = static_cast<GeoPot*>(
      malloc(GEOPOTSTART * sizeof(GeoPot)));
  gix->gc = static_cast<GeoCoordinate*>(malloc(GEOSLOTSTART * sizeof(GeoCoordinate)));
...
GeoResults* GeoResultsCons(int alloc) {
  ...
  gres = static_cast<GeoResults*>(
      malloc(sizeof(GeoResults)));
  sa = static_cast<int*>(
      malloc(alloc * sizeof(int)));
  dd = static_cast<double*>(
      malloc(alloc * sizeof(double)));
...
int GeoResultsGrow(GeoResults* gr) {
  ...
  sa = static_cast<int*>(
    realloc(gr->slot, newsiz * sizeof(int)));
  dd = static_cast<double*>(
    realloc(gr->snmd, newsiz * sizeof(double)));
  if ((sa == NULL) || (dd == NULL)) {
...
GeoCoordinates* GeoAnswers(GeoIx* gix, GeoResults* gr, bool returnDistances) {
  ...
  ans = static_cast<GeoCoordinates*>(
      malloc(sizeof(GeoCoordinates)));
  gc = static_cast<GeoCoordinate*>(malloc(gr->pointsct * sizeof(GeoCoordinate)));
...
int GeoIndexNewSlot(GeoIx* gix) {
  ...
  if (gix->gc[0].latitude == 0.0) {
    ...
    int newslotct = (int)x;
    gc = static_cast<GeoCoordinate*>(realloc(gix->gc, newslotct * sizeof(GeoCoordinate)));
